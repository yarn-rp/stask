...
export function registerAgents({ projectSlug, agents, leadId, slackAccounts, openclawDefaults }) {
  ...
  if (existingDefaults.maxConcurrent === undefined) {
    ops.push({
      path: 'agents.defaults.maxConcurrent',
      value: ocDefaults.maxConcurrent || 4,
    });
  }
  ...
  const leadMaxConcurrent = ocDefaults.leadMaxConcurrent ?? 8;
  ...
  for (const agent of agents) {
    ...
    if (agent.id === leadId) {
      ...
      // Lead needs extra concurrency because worker announces fan in here.
      entry.maxConcurrent = leadMaxConcurrent;
    } else {
