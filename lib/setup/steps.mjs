...
export async function stepBootstrapTask(s, ctx) {
  ...
  // 5. Build the task's native Slack URL. We prefer the list-record URL
  // (https://<ws>/lists/<team>/<list>?record_id=<rec>) because it opens
  // inside the Slack desktop/mobile client instead of bouncing to the
  // browser like the app.slack.com thread URL does. Slack needs a moment
  // to index the row id; retry briefly if `stask show` hasn't reported it.
  let rowMatch = null;
