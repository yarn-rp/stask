...
export function getDb() {
  ...
  if (isNew) {
    ...
    // Migrate: add spec_approved_at + spec_approved_by columns. These persist
    // the "human ticked the spec_approved checkbox in Slack" signal so every
    // gate (subtask creation, heartbeat actions, In-Progress transition) can
    // refuse to start work until the human has explicitly approved.
    const cols2 = _db.prepare('PRAGMA table_info(tasks)').all().map(c => c.name);
    if (!cols2.includes('spec_approved_at')) {
      _db.exec('ALTER TABLE tasks ADD COLUMN spec_approved_at TEXT');
    }
    if (!cols2.includes('spec_approved_by')) {
      _db.exec('ALTER TABLE tasks ADD COLUMN spec_approved_by TEXT');
    }
    ...
    if (tableSql?.sql && (!tableSql.sql.includes('Backlog') || !tableSql.sql.includes('Improvement') || tableSql.sql.includes('spec          TEXT NOT NULL'))) {
      ...
      _db.exec(`
        ...
        INSERT INTO tasks_new SELECT
          ...
          spec_approved_at, spec_approved_by,
    ...
    // Multi-repo + Jira columns. `primary_repo` records which configured
    // repo this task is conceptually about; `jira_key` is the Jira ticket
    // ID (e.g. ACME-123) used for branch-detection lookups. Run AFTER any
    // table recreate so the new columns survive.
    const cols3 = _db.prepare('PRAGMA table_info(tasks)').all().map(c => c.name);
    if (!cols3.includes('primary_repo')) {
      _db.exec('ALTER TABLE tasks ADD COLUMN primary_repo TEXT');
    }
    if (!cols3.includes('jira_key')) {
      _db.exec('ALTER TABLE tasks ADD COLUMN jira_key TEXT');
    }

    // Migrate `worktree` column from legacy "branch (path)" string to a
    // JSON array of {repo, branch, path}. One row at a time so we don't
    // touch already-migrated rows.
    const wtRows = _db.prepare(
      "SELECT task_id, worktree FROM tasks WHERE worktree IS NOT NULL AND worktree != ''"
    ).all();
    for (const row of wtRows) {
      const v = row.worktree.trim();
      if (v.startsWith('[')) continue;  // already JSON
      const m = v.match(/^(.+?)\s+\((.+)\)$/);
      if (!m) continue;  // unrecognized — leave alone
      const branch = m[1].trim();
      const wtPath = m[2].trim();
      const repoKey = path.basename(wtPath);
      const json = JSON.stringify([{ repo: repoKey, branch, path: wtPath }]);
      _db.prepare('UPDATE tasks SET worktree = ? WHERE task_id = ?').run(json, row.task_id);
    }

    // Migrate `inbox_subs.source_type` CHECK constraint to include 'jira'.
    // SQLite can't alter CHECK constraints, so recreate the table.
    const inboxSql = _db.prepare(
      "SELECT sql FROM sqlite_master WHERE type='table' AND name='inbox_subs'"
    ).get();
    if (inboxSql?.sql && !inboxSql.sql.includes("'jira'")) {
      _db.exec('PRAGMA foreign_keys = OFF');
      _db.exec(`
        CREATE TABLE inbox_subs_new (
          sub_id        TEXT PRIMARY KEY,
          source_type   TEXT NOT NULL CHECK (source_type IN ('github', 'linear', 'jira')),
          target_id     TEXT NOT NULL,
          filters       TEXT,
        ...
        INSERT INTO inbox_subs_new SELECT * FROM inbox_subs;
        DROP TABLE inbox_subs;
        ALTER TABLE inbox_subs_new RENAME TO inbox_subs;
      `);
      _db.exec('PRAGMA foreign_keys = ON');
    }
  }
...
/**
 * Parse the `worktree` column into an array of `{repo, branch, path}`.
 * Accepts: JSON array (canonical), legacy "branch (path)" string, null.
 * Returns `[]` for null/empty so callers can `.length` check uniformly.
 */
export function parseWorktrees(value) {
  ...
  if (trimmed.startsWith('[')) {
    try {
      ...
      if (Array.isArray(parsed)) {
        return parsed.filter((e) => e && e.branch && e.path)
          .map((e) => ({ repo: e.repo || path.basename(e.path), branch: e.branch, path: e.path }));
      }
    } catch {}
    return [];
  }
  ...
  if (m) {
    const branch = m[1].trim();
    const wtPath = m[2].trim();
    return [{ repo: path.basename(wtPath), branch, path: wtPath }];
  }
  return [];
}

/**
 * Back-compat: returns the first `{branch, path}` entry, or null. Existing
 * callers that only handle a single worktree continue to work and get the
 * primary (first) repo's worktree.
 */
export function parseWorktreeValue(value) {
  const all = parseWorktrees(value);
  if (all.length === 0) return null;
  const { branch, path: p } = all[0];
  return { branch, path: p };
}

/**
 * Format an array of `{repo, branch, path}` entries into the JSON value
 * stored in the `worktree` column.
 */
export function formatWorktrees(entries) {
  if (!Array.isArray(entries) || entries.length === 0) return null;
  const cleaned = entries.map((e) => ({
    repo: e.repo || path.basename(e.path),
    branch: e.branch,
    path: e.path,
  }));
...
/**
 * Back-compat: format a single (branch, path) into the canonical JSON
 * shape. Used by callers that haven't migrated to the multi-entry API.
 */
export function formatWorktreeValue(branch, worktreePath, repo) {
  return formatWorktrees([{ repo: repo || path.basename(worktreePath), branch, path: worktreePath }]);
}

/**
 * Look up a task's worktree entry for a specific repo key (basename of
 * the configured repo path). Falls back to parent if `taskId` is a
 * subtask. Returns null if no match.
 */
export function getWorktreeForRepo(taskId, repoKey) {
  const task = findTask(taskId);
  if (!task) return null;
  const own = parseWorktrees(task['Worktree']);
  const fromOwn = own.find((e) => e.repo === repoKey);
  if (fromOwn) return fromOwn;
  if (task['Parent'] && task['Parent'] !== 'None') {
    const parent = findTask(task['Parent']);
    if (parent) {
      const parentWts = parseWorktrees(parent['Worktree']);
      return parentWts.find((e) => e.repo === repoKey) || null;
    }
  }
  return null;
}
...
/**
 * Get all worktrees for a task (or its parent if it's a subtask).
 * Returns the array of `{repo, branch, path}` entries.
 */
export function getParentWorktrees(taskId) {
  const task = findTask(taskId);
  if (!task) return [];
  const own = parseWorktrees(task['Worktree']);
  ...
  if (task['Parent'] && task['Parent'] !== 'None') {
    const parent = findTask(task['Parent']);
    if (parent) return parseWorktrees(parent['Worktree']);
  }
  return [];
}

/**
 * Get the primary worktree for a task (or its parent if it's a subtask).
 * Back-compat: returns `{branch, path}` like the old single-worktree API.
 * Picks the entry matching the task's `primary_repo` if set, otherwise
 * the first entry.
 */
export function getParentWorktree(taskId) {
  ...
  const own = parseWorktrees(task['Worktree']);
  const pickFrom = (arr, primary) => {
    if (arr.length === 0) return null;
    if (primary) {
      const match = arr.find((e) => e.repo === primary);
      if (match) return match;
    }
    return arr[0];
  };
  const fromOwn = pickFrom(own, task['primary_repo']);
  if (fromOwn) return { branch: fromOwn.branch, path: fromOwn.path };

  if (task['Parent'] && task['Parent'] !== 'None') {
    const parent = findTask(task['Parent']);
    if (parent) {
      const parentWts = parseWorktrees(parent['Worktree']);
      const fromParent = pickFrom(parentWts, parent['primary_repo']);
      if (fromParent) return { branch: fromParent.branch, path: fromParent.path };
    }
  }
  return null;
}
...
function rowToTask(row) {
  return {
    ...
    'primary_repo': row.primary_repo || null,
