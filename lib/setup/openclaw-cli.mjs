...
/**
 * `openclaw models list --json`. Returns the array of model ids the
 * user has registered locally (e.g. "ollama/llama3:8b",
 * "anthropic/claude-sonnet-4-6"). Returns [] if the CLI is unreachable
 * or no models are registered — the caller should fall back to the
 * manifest's recommended default in that case.
 */
export function modelsList() {
  try {
    const out = runOpenclaw(['models', 'list', '--json']).trim();
    if (!out) return [];
    const parsed = JSON.parse(out);
