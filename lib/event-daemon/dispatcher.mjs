...
export async function dispatch(event, ctx) {
  ...
  if (matching.length === 0) {
    // Surface the silent skip — without this log a `file_change` whose
    // file_id should match listId but doesn't (variant payload, frozen
    // ctx mismatch, etc.) disappears with no trace.
    logger.info(
      `[dispatcher] no handler matched ${eventType}` +
      (event.file_id ? ` (file_id=${event.file_id})` : '') +
      ` registered=[${HANDLERS.map(h => `${h.name}:${h.eventType}`).join(',')}]`,
    );
    return;
  }
