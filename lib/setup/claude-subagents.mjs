...
/**
 * Stask lifecycle skills for a role. These let the Claude session close its
 * own work via the stask CLI (e.g. `stask subtask done`, `stask qa submit`).
 * Sourced from <stask>/skills/stask-*. Falls back to sensible defaults if the
...
function getStaskSkillsForRole(roleId, manifests) {
  ...
  if (roleId === 'lead') return ['stask-general', 'stask-lead'];
  if (roleId === 'qa') return ['stask-general', 'stask-qa'];
  return ['stask-general', 'stask-worker'];
}
