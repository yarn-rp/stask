...
/**
 * Get all roles. Solo-agent templates have exactly one entry (the lead).
 */
export function getRoles(agents) {
  return Object.entries(agents).map(([id, manifest]) => ({ id, ...manifest }));
}

/**
 * Get the lead role (role: "lead" in the agent manifest).
 */
export function getLeadRole(agents) {
  const entry = Object.entries(agents).find(([_, m]) => m.role === 'lead');
  if (!entry) throw new Error('No lead role found (manifest.role === "lead")');
  return { id: entry[0], ...entry[1] };
}

/**
 * Worker roles are gone in the solo-agent model. Kept as a no-op for callers
 * that still iterate over workers; returns an empty array.
 */
