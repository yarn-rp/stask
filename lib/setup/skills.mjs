...
const FALLBACK_ROLE_SKILLS = {
  lead: ['gsd', 'planning-files', 'requirements-analysis', 'technical-spec-design', 'code-review', 'critical-code-reviewer', 'security-auditor', 'agentic-workflow-automation', 'pre-mortem-analyst', 'feature-specification', 'coding-agent'],
};
  ...
  if (process.env.STASK_SKIP_SKILLS_INSTALL) {
    ...
    for (const skill of lists.stask) {
      ...
      if (fs.existsSync(src)) {
        try { fs.symlinkSync(src, dest, 'dir'); symlinked.push(skill); }
        catch { failed.push(skill); }
      }
    }
