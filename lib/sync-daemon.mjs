#!/usr/bin/env node
/**
 * sync-daemon.mjs — Long-running drift reconciler (safety net).
 *
 * Runs `runSyncCycle()` at a configurable interval (default: 3600s / 1 hour).
 * This is intentionally coarse — the Slack Socket Mode event daemon handles
 * real-time Slack→DB updates. This process catches anything the event stream
 * may have missed (dropped envelope, daemon downtime, config-drift).
 *
 ...
 * Logs to .stask/logs/sync-daemon.log.
 */
...
const INTERVAL_MS = (CONFIG.syncIntervalSeconds || 3600) * 1000;
