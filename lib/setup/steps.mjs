...
import { log, pc, dimPath, select, text } from './prompt.mjs';
...
export async function stepInbox(s, ctx) {
  ...
  const inboxChoice = choices.length > 1 ?
    await select({ message: 'What would you like to monitor?', options: choices }) :
    choices[0].value === 'none' ? 'none' : choices[0].value;
    ...
    try {
      ...
      if (existingIdx >= 0) {
        ...
        jobs.jobs[existingIdx].payload = {
          kind: 'agentTurn',
          message: 'Run inbox pollerd to fetch GitHub/Linear events.',
          timeoutSeconds: 300,
        };
      } else {
        // Create new job
        ...
        jobs.jobs.push({
          ...
          agentId: ctx.slug.split('-').includes('project') ? ctx.slug.replace('-project', '') : ctx.slug,
          name: 'inbox-pollerd',
