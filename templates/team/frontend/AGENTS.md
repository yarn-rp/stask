...
- Claude Code invocation:
  ```bash
  cd {{PROJECT_ROOT}} && claude --agent {{FRONTEND_NAME_LOWER}} -p 'task description with design rules'
  ```
...
Every Claude session you open runs as **you** — the `{{FRONTEND_NAME_LOWER}}` subagent — with your role playbook preloaded from `{{PROJECT_ROOT}}/.claude/agents/{{FRONTEND_NAME_LOWER}}.md`:

```bash
cd {{PROJECT_ROOT}} && claude --agent {{FRONTEND_NAME_LOWER}} -p 'Your task here'
```

### Rules

- **Always use Claude Code first** — it is the primary tool for all code work
- **Always pass `--agent {{FRONTEND_NAME_LOWER}}`** — this loads your identity and preloaded skills. A bare `claude` has no role context.
- **You orchestrate, it executes** — review output before handing off
- Your subagent definition lives at `{{PROJECT_ROOT}}/.claude/agents/{{FRONTEND_NAME_LOWER}}.md`; shared skills at `{{PROJECT_ROOT}}/.claude/skills/`
