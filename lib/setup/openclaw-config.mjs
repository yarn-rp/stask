 ...
 */
export function registerAgents({ projectSlug, agents, leadId, slackAccounts, openclawDefaults, acpDefaults }) {
  ...
  // Only keep a default heartbeat cadence if one is explicitly provided — the
  // new architecture has no team-wide heartbeat; the lead's manifest sets its
  // own cron schedule. We preserve whatever was previously written so we don't
  // clobber user customizations.
  if (ocDefaults.heartbeat?.every) defaults.heartbeat.every = ocDefaults.heartbeat.every;
  ...
  // ── ACP defaults (Codex-via-acpx coding layer) ───────────────────
  if (acpDefaults) {
    if (!defaults.acp) defaults.acp = {};
    defaults.acp.enabled = acpDefaults.enabled ?? defaults.acp.enabled ?? true;
    defaults.acp.cli = acpDefaults.cli || defaults.acp.cli || 'acpx';
    defaults.acp.agent = acpDefaults.agent || defaults.acp.agent || 'codex';
    defaults.acp.pingIntervalSeconds = acpDefaults.pingIntervalSeconds ?? defaults.acp.pingIntervalSeconds ?? 60;
    defaults.acp.hangTimeoutMinutes = acpDefaults.hangTimeoutMinutes ?? defaults.acp.hangTimeoutMinutes ?? 3;
    defaults.acp.ttlSeconds = acpDefaults.ttlSeconds ?? defaults.acp.ttlSeconds ?? 0;
    defaults.acp.fallback = acpDefaults.fallback || defaults.acp.fallback || 'sessions_spawn';
  }
