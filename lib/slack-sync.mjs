...
import { ensureTable as ensureActiveSessionsTable } from './session-tracker.mjs';
...
const OPTION_TO_SPEC_APPROVED = Object.fromEntries(
  Object.entries(SLACK.specApprovedOptions || {}).map(([name, optId]) => [optId, name])
);
...
export function parseSlackItem(item) {
  ...
  for (const field of item.fields) {
    ...
    // ── Spec Approved (single-select — pull-only trigger, never stored in DB) ──
    // Modeled as a select to work around Slack not firing file_change for checkbox
    // edits. Treat the "Approved" option as a positive trigger.
    if (fieldName === 'spec_approved') {
      const optId = field.select ? field.select[0] : null;
      if (optId && OPTION_TO_SPEC_APPROVED[optId] === 'Approved') {
        result._spec_approved = true;
      }
    }
  }
...
/**
 * Retry a Slack operation with exponential backoff.
 * Returns { ok, error } - ok is true on success, error contains the error message on failure.
 */
function retrySlackOperation(fn, { source, operation, taskId, metadata } = {}) {
  const MAX_RETRIES = 3;
      ...
      fn().then(result => {
        ...
        if (attempt <= MAX_RETRIES - 1) {
          if (attempt === MAX_RETRIES - 1) {
            ...
            attempt++;
            setTimeout(tryOnce, BACKOFF_BASE * (2 ** attempt));
          }
        } else {
          resolve({ ok: false, error: err.message });
        }
...
export async function runSyncCycle() {
  ...
  for (const task of allTasks) {
    ...
    try {
      const syncResult = await syncOneTask(db, libs, task, slackItem, threadNotifications);
 ...
 */
async function syncOneTask(db, libs, task, slackItem, threadNotifications = []) {
  const taskId = task['Task ID'];
 ...
 * Delegates to reconcileSlackItem (lib/slack-reconcile.mjs) so the poll path
 * and the event-daemon path share identical logic.
 */
async function pullFromSlack(db, libs, taskId, task, slackItem, slackTs, threadNotifications = []) {
  return reconcileSlackItem(slackItem, { source: 'poll', db, libs });
}
