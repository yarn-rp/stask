...
/**
 * Subtask cannot move to In-Progress unless its parent is already
 * In-Progress (or beyond). Parent tasks pass automatically.
 * Scope: 'any' — applies to both parent and subtask transitions.
 */
function requireParentInProgress(task, libs) {
  // Parent tasks always pass — they have no parent to check
  if (task['Parent'] === 'None') return { pass: true };

  const parentTask = libs.trackerDb.findTask(task['Parent']);
  if (!parentTask) {
    return { pass: false, reason: `Cannot transition subtask ${task['Task ID']} to In-Progress: parent task ${task['Parent']} not found` };
  }

  const allowedStatuses = ['In-Progress', 'Testing', 'Ready for Human Review'];
  if (!allowedStatuses.includes(parentTask['Status'])) {
    return { pass: false, reason: `Cannot transition subtask ${task['Task ID']} to In-Progress: parent task ${parentTask['Task ID']} is not yet In-Progress (current status: ${parentTask['Status']})` };
  }
...
/**
 * Task must have been explicitly approved by the human via the
 * spec_approved checkbox in Slack. The slack-reconcile path persists
 * this as `spec_approved_at`. Without that signal, no agent — not even
 * the lead — may start work, even if the task is no longer assigned to
 * the human.
 *
 * Only applies when coming FROM To-Do (initial approval gate). Review
 * cycles (RHR → In-Progress) skip this — by then the task is long past
 * the initial approval gate.
 */
function requireApproved(task) {
  if (task['Status'] !== 'To-Do') return { pass: true };
  if (!task['spec_approved_at']) {
    return {
      pass: false,
      reason: `Task has not been approved. Tick the spec_approved checkbox in Slack — only the human can authorize work to start.`,
    };
  }
  return { pass: true };
}
...
const GUARDS = {
  'To-Do': [
    { name: 'require_spec', type: 'check', scope: 'parent', fn: requireSpec },
  ],
  'In-Progress': [
    { name: 'require_subtasks',          type: 'check', scope: 'parent', fn: requireSubtasks },
    { name: 'require_approved',          type: 'check', scope: 'parent', fn: requireApproved },
    { name: 'require_parent_in_progress', type: 'check', scope: 'any',    fn: requireParentInProgress },
    { name: 'setup_worktree',            type: 'setup', scope: 'parent', fn: setupWorktree },
  ],
  'Testing': [
    { name: 'all_subtasks_done', type: 'check', scope: 'parent', fn: allSubtasksDone },
    { name: 'worktree_clean',    type: 'check', scope: 'parent', fn: worktreeClean },
    { name: 'worktree_pushed',   type: 'check', scope: 'parent', fn: worktreePushed },
  ],
  'Ready for Human Review': [
    { name: 'worktree_clean',    type: 'check', scope: 'parent', fn: worktreeClean },
    { name: 'worktree_pushed',   type: 'check', scope: 'parent', fn: worktreePushed },
    { name: 'require_pr',       type: 'check', scope: 'parent', fn: requirePR },
    { name: 'setup_pr',          type: 'setup', scope: 'parent', fn: setupPR },
  ],
  'Done': [
    { name: 'block_cli_done',    type: 'check', scope: 'parent', fn: blockCliDone },
  ],
};
...
export function runGuards(task, newStatus, libs) {
  ...
  // Filter guards by scope: parent-only guards are skipped for subtasks
  const applicable = guards.filter(g => isParent || g.scope === 'any');
  if (applicable.length === 0) return { ok: true, failures: [] };

  const checks = applicable.filter(g => g.type === 'check');
  const setups = applicable.filter(g => g.type === 'setup');
  const failures = [];
