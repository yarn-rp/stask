...
import { appendToExclude } from './git-exclude.mjs';
...
export async function stepClaudeSubagents(s, ctx, { projectName, humanName, manifests, agentRoles }) {
  ...
  if (res.agentsSkipped && res.agentsSkipped.length) {
    log.warn(`  ${pc.yellow('!')} ${res.agentsSkipped.length} agent file(s) already existed and were left untouched (team-committed): ${res.agentsSkipped.join(', ')}`);
  }

  // Hide stask's `.claude/` additions and `.stask/` from each work
  // repo's `git status` via per-clone `.git/info/exclude`. Honors
  // team-committed files by only listing paths we actually wrote.
  const excludePatterns = ['.stask/', ...(res.writtenPaths || [])];
  const reposToExclude = ctx.repos && ctx.repos.length
    ? ctx.repos
    : [{ path: ctx.repoPath }];
  for (const repo of reposToExclude) {
    try {
      appendToExclude(repo.path || repo, excludePatterns);
    } catch (err) {
      log.warn(`  ${pc.yellow('!')} Could not update .git/info/exclude in ${repo.path || repo}: ${err.message}`);
    }
  }
}
...
/**
 * Setup inbox subscriptions (GitHub/Linear/Jira polling).
 *
 * ctx may carry ctx.jiraKey (string|null) from Phase 0 to seed the Jira
...
export async function stepInbox(s, ctx) {
  ...
  try {
    execFileSync('jira', ['version'], { encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'], timeout: 5000 });
    jiraAvailable = true;
  } catch {
    // jira not available - we'll warn but continue
  }
  ...
  if (jiraAvailable) choices.push({ value: 'jira', label: 'Jira projects', hint: 'Monitor Jira issues' });
  ...
  // For Jira, use the jiraKey from Phase 0 as default if available
  if (sourceType === 'jira' && ctx.jiraKey) {
    defaultTarget = ctx.jiraKey;
  }

  let targetPromptMessage;
  let targetPromptPlaceholder;
  if (sourceType === 'github') {
    targetPromptMessage = 'GitHub repositories (comma-separated owner/repo)';
    targetPromptPlaceholder = defaultTarget || 'owner/repo1,owner/repo2';
  } else if (sourceType === 'linear') {
    targetPromptMessage = 'Linear project keys (comma-separated)';
    targetPromptPlaceholder = 'PROJ1,PROJ2';
  } else {
    targetPromptMessage = 'Jira project keys (comma-separated)';
    targetPromptPlaceholder = defaultTarget || 'PROJ1,PROJ2';
  }

  const targetInput = await text({
    message: targetPromptMessage,
  ...
  for (const target of targets) {
    ...
    if (sourceType === 'github' && ghAvailable) {
    ...
    } else if (sourceType === 'jira' && jiraAvailable) {
      try {
        execFileSync('jira', ['project', 'list'], {
          encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'], timeout: 10000,
        });
      } catch (err) {
        const detail = (err.stderr || err.stdout || err.message || '').toString().trim().split('\n').pop();
        log.warn(`  ${pc.yellow('Skip')} ${pc.dim(target)} - ${detail || 'cannot access Jira'} (check jira init)`);
        valid = false;
      }
      ...
      valid = false;
    } else if (!jiraAvailable && sourceType === 'jira') {
      log.warn(`  ${pc.yellow('Skip')} ${pc.dim(target)} - Jira CLI not installed`);
      valid = false;
    }
  ...
  if (subscribedCount > 0) {
    try {
      const cronExpr = sourceType === 'linear' ? '*/15 * * * *' : sourceType === 'jira' ? '*/10 * * * *' : '*/5 * * * *';
      cronUpsert({
        agentId: ctx.slug,
        name: 'inbox-pollerd',
        cronExpr,
        description: 'Inbox polling daemon for GitHub/Linear/Jira events',
