...
- **QA Exploration ({{QA_NAME}}):** Prompt for automated vs manual test coverage and test strategy. **Note: QA is a phase gate, not subtasks — {{QA_NAME}}'s exploration informs the QA phase plan, not subtask creation.**
...
1. **QA Cycle** → {{QA_NAME}} tests against ACs. **QA is a phase gate, not subtasks** — it happens after all subtasks are done. If FAIL, transition back to In-Progress, create fix subtasks, and repeat.
...
6. **QA is a separate phase, NOT subtasks:** Subtasks are for development work only ({{BACKEND_NAME}}/{{FRONTEND_NAME}} implementation). QA happens AFTER implementation via the Testing phase (Phase 6). {{QA_NAME}} tests against acceptance criteria as a phase gate — do NOT create QA subtasks. The QA phase is triggered when workers mark subtasks done, not by creating subtasks for {{QA_NAME}}.
