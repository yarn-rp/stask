...
export async function stepCron(s, ctx, agentManifests) {
  s.start('Setting up heartbeat cron jobs (openclaw cron)...');
  const agents = Object.entries(ctx.agents);

  const res = setupCronJobs(agents, agentManifests, ctx.slug);
  if (res.gatewayDown) {
    s.stop(pc.yellow('OpenClaw gateway unreachable — cron sync skipped'));
    log.warn(`  Re-run ${pc.cyan('stask setup --only cron')} after starting the gateway.`);
    return;
  }
  const parts = [
    `${pc.bold(res.created.length)} created`,
    `${pc.bold(res.updated.length || 0)} updated`,
  ];
  if (res.removed?.length) parts.push(`${pc.bold(res.removed.length)} removed`);
  s.stop(parts.join(', ') + ' heartbeats');
}
...
export async function stepOpenclaw(s, ctx, agentModels, teamManifest, providedSlackAccounts) {
  ...
  const res = registerAgents({
    ...
    acpDefaults: teamManifest?.acp,
    staskConfigPath: ctx.staskConfigPath,
    humanSlackUserId: ctx.humanUserId,
  });
  const parts = [`${pc.bold(res.added.length)} registered`];
  if (res.skipped?.length) parts.push(`${pc.bold(res.skipped.length)} already present`);
  if (res.slackAccounts?.added?.length) parts.push(`${pc.bold(res.slackAccounts.added.length)} Slack accounts`);
  if (res.slackAccounts?.trustApplied?.length) parts.push(`${pc.bold(res.slackAccounts.trustApplied.length)} trust policies`);
  s.stop(parts.join(', '));
}
