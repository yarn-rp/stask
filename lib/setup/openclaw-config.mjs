...
import {
  ...
  enableAcpxPluginWithAutoApprove,
...
export function registerAgents({
  ...
  // ── 0. Enable the acpx plugin with auto-approve for writes ────────
  // Without this, Sub-agent({ runtime: "acp" }) calls prompt for every
  // file edit and the solo agent has no way to answer — turns stall.
  try {
    enableAcpxPluginWithAutoApprove();
  } catch (err) {
    // eslint-disable-next-line no-console
    console.warn(`  ! enableAcpxPluginWithAutoApprove failed: ${err.message.split('\n')[0]}`);
  }
