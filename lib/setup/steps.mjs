...
export async function stepBootstrapTask(s, ctx) {
  ...
  // 5. Build the task thread URL (required — welcome message links to it).
  // Slack needs a moment to index the list-item thread; retry briefly if
  // the Thread line isn't in `stask show` yet.
  let threadMatch = null;
  for (let attempt = 0; attempt < 6; attempt++) {
    const showOut = run('show', taskId);
    threadMatch = showOut.match(/Thread:\s+(\S+):(\S+)/);
    if (threadMatch) break;
    await sleep(2000);
  }
  if (!threadMatch) {
    const err = new Error(`Could not find Slack thread for ${taskId} after retries; stask show did not report a Thread line.`);
    s.stop(pc.red(`Bootstrap task created but thread lookup failed: ${err.message}`));
    throw err;
  }
  if (!ctx.wsInfo) ctx.wsInfo = await getWorkspaceInfo(ctx.leadToken);
  const [, channelId, threadTs] = threadMatch;
  const taskThreadUrl = `https://app.slack.com/client/${ctx.wsInfo.teamId}/${channelId}/thread/${channelId}-${threadTs}`;
