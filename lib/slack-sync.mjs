...
// ─── Canvas spec content pull ─────────────────────────────────────

/**
 * Pull Canvas content changes to the local spec file.
 * Called when a spec is of type 'canvas' and the Canvas has been edited.
 *
 * @param {object} libs - Workspace libs
 * @param {string} localSpecPath - Absolute path to the local spec .md file
 * @param {string} canvasId - Slack Canvas ID
 * @returns {Promise\u003cboolean\u003e} true if content was pulled, false if no change
 */
export async function pullCanvasSpecContent(libs, localSpecPath, canvasId) {
  const { getCanvasFileInfo, downloadCanvasContent } = libs.slackApi;
  const { canvasToMarkdown } = await import('./canvas-format.mjs');
  const { extractFrontmatter, injectFrontmatter, updateFrontmatter } = await import('./yaml-frontmatter.mjs');

  const info = await getCanvasFileInfo(canvasId);

  // Check if the Canvas has been edited since our last pull
  try {
    const localStat = fs.statSync(localSpecPath);
    const localMtime = Math.floor(localStat.mtimeMs / 1000);
    if (info.editTimestamp \u003c= localMtime) {
      return false; // Canvas not newer than local
    }
  } catch {
    // File doesn't exist locally yet — pull it
  }
  ...
  // Preserve frontmatter if the local file has it
  let existingFrontmatter = {};
  try {
    const existing = fs.readFileSync(localSpecPath, 'utf-8');
    const parsed = extractFrontmatter(existing);
  ...
  const finalContent = injectFrontmatter(md, updatedFrontmatter);
  fs.writeFileSync(localSpecPath, finalContent, 'utf-8');

  return true;
}

/**
 * Sync Canvas spec content for all tasks with canvas-type specs.
 * Called as part of the regular sync cycle.
 *
 * @returns {{ pulled: string[], errors: string[] }}
 */
export async function syncCanvasSpecs() {
  const libs = await getWorkspaceLibs();
  const db = libs.trackerDb.getDb();
  const result = { pulled: [], errors: [] };

  // Find all tasks with canvas-type specs
  const tasks = libs.trackerDb.getAllTasks();
  for (const task of tasks) {
    const specValue = task['Spec'];
    if (!specValue) continue;

    const parsed = parseSpecValue(specValue);
    if (!parsed || parsed.type !== 'canvas' || !parsed.canvasId) continue;

    // Resolve local file path
    const ws = CONFIG.specsDir;
    const localSpecPath = path.resolve(ws, 'shared', parsed.filename);

    try {
      const pulled = await pullCanvasSpecContent(libs, localSpecPath, parsed.canvasId);
      if (pulled) {
        result.pulled.push(task['Task ID']);
      }
    } catch (err) {
