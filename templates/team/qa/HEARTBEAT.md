...
1. Poll for work:
   ...
   The JSON also includes a slim `summary: { totalAssignedOpen, awaitingApproval }` — informational only. `pendingTasks` is the actionable list (actions: `qa`, `resume`); treat every entry the same.
