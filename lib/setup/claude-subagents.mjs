...
export function scaffoldClaudeSubagents({ repoPath, projectName, projectSlug, humanName, openclawHome, agents, manifests }) {
  ...
  for (const { name, roleId } of agents) {
    ...
    for (const skill of skillList) {
      ...
      if (fs.existsSync(dest)) continue; // already copied for another agent
    ...
    // Render the agent file with its skill list in YAML frontmatter.
    const skillsYaml = skillList.map((s) => `  - ${s}`).join('\n');
  ...
  return {
    agentsWritten,
    skillsCopied: skillsCopied.size,
