...
export async function stepActivateListChannel(s, ctx) {
  const { getWorkspaceLibs } = await import('../env.mjs');
  const { slackApi } = await getWorkspaceLibs();
  ...
  try {
    const hist = await slackApi.getChannelHistory(channelId, { limit: 1 });
    if ((hist?.messages || []).length > 0) {
      s.stop(`${pc.green('\u2713')} List comment thread channel already activated`);
      return;
    }
  } catch (err) {
    // channel_not_found is expected on a fresh list — fall through to
    // warmup-row flow. Anything else: surface but don't block.
    if (err?.slackError !== 'channel_not_found') {
  ...
  try {
    ...
    const created = await slackApi.createListRow(listId, initialFields, null);
    warmupRowId = created.item?.id;
    if (!warmupRowId) throw new Error('createListRow returned no row id');
  ...
  while (Date.now() < deadline) {
    try {
      const hist = await slackApi.getChannelHistory(channelId, { limit: 1 });
      if ((hist?.messages || []).length > 0) { activated = true; break; }
    } catch (err) {
      if (err?.slackError !== 'channel_not_found') {
        log.warn(`  ${pc.yellow('Activation check')} ${pc.dim(err.message || String(err))}`);
      }
  ...
  if (warmupRowId) {
    try {
      await slackApi.slackApiRequest('POST', '/slackLists.items.delete', {
        list_id: listId, id: warmupRowId,
      });
    } catch (err) {
