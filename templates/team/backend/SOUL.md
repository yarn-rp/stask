...
3. Open a Claude Code session as yourself (your playbook preloads from `.claude/agents/{{BACKEND_NAME_LOWER}}.md`). **Follow the `stask-coding` skill** for the invocation recipe and stask-framework prompt template — don't free-form when a task, spec, and subtasks are in play.
