...
export async function shareListInChannel({ botToken, listId, channelId, userIds, humanUserId }) {
  ...
  if (humanUserId) {
    const res = await slackPost(botToken, 'slackLists.access.set', {
      list_id: listId,
      user_ids: [humanUserId],
      access_level: 'owner',
    });
    if (!res.ok) accessErrors.push({ userId: humanUserId, accessLevel: 'owner', error: res.error });
  }

  // Grant write access to agents one-by-one so a single bad ID doesn't sink
  // the rest, and so we can surface per-user errors back to the caller.
  const agentIds = (userIds || []).filter((id) => id && id !== humanUserId);
  for (const userId of agentIds) {
    ...
    if (!res.ok) accessErrors.push({ userId, accessLevel: 'write', error: res.error });
  }
  ...
  return { ok: true, listUrl, accessErrors: accessErrors.length ? accessErrors : undefined };
}
