...
export async function stepBootstrapTask(s, ctx) {
  ...
  try {
    const out = run('create', 'Bootstrap: Team Explore Codebase & Set Up Agents', '--overview',
  ...
  try {
    run('spec-update', taskId, 'shared/specs/bootstrap-team.md');
  } catch (err) {
    log.warn(pc.dim(`  Spec attach skipped: ${err.message}`));
  }

  // 3. Create subtasks for each agent role
  ...
  for (const [agent, title] of subtasks) {
    try {
      run('subtask', 'create', taskId, title, '--assign', agent);
    } catch (err) {
      log.warn(pc.dim(`  Subtask "${title}" failed: ${err.message}`));
    }
  }

  // 4. Transition to To-Do
  try {
    run('transition', taskId, 'To-Do');
  } catch (err) {
    log.warn(pc.dim(`  Transition failed: ${err.message}`));
  }
