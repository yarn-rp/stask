...
import { getNameBySlackUserId, getLeadAgent, getHuman } from './roles.mjs';
...
function pullFromSlack(db, libs, taskId, task, slackItem, slackTs, threadNotifications = []) {
  ...
  // Spec approval via Slack checkbox: reassign from human to lead AND force transition
  let wasSpecApproved = false;
  if (parsed._spec_approved && task['Status'] === 'To-Do' && task['Assigned To'] === getHuman()) {
  ...
  // Special handling for spec approval: this is a key milestone
  if (wasSpecApproved) {
    ...
    if (parts.length > 0) {
      notificationMessage = `*${taskId}* updated via Slack: ${parts.join(', ')}`;
    }
  }
